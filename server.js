...
// In-memory stores (resets when server sleeps/restarts)
const tickStore = {};   // { symbol: [ { quote, digit, timestamp }, ... ] }
const tradeLog = [];    // [ { ... }, ... ]
...
app.get("/", (req, res) => {
    res.json({
        status: "ok",
        endpoints: ["/api/balance", "/api/ticks", "/api/ticks/:symbol", "/api/analysis/:symbol", "/api/trades"]
    });
});

// Receive tick from frontend
app.post("/api/ticks", (req, res) => {
    const { symbol, quote } = req.body;
    if (!symbol || quote === undefined) {
        return res.status(400).json({ error: "symbol and quote required" });
    }
    ...
    // Cap at 2000 ticks per symbol to prevent memory bloat
    if (tickStore[symbol].length > 2000) {
        tickStore[symbol] = tickStore[symbol].slice(-2000);
    }

    res.json({ success: true, count: tickStore[symbol].length });
});

// Fetch recent ticks (frontend signal engine calls this)
...
// Fetch analysis (frontend analysis card calls this)
app.get("/api/analysis/:symbol", (req, res) => {
    ...
    if (sample.length === 0) {
        return res.status(404).json({ error: "No tick data for this symbol yet.", lookback: 0 });
    }
    ...
    let hotDigit = 0, hotPct = 0;
    let coldDigit = 0, coldPct = 100;
    ...
    // Max streak
    let maxStreak = { digit: null, length: 0 };
    let current = { digit: null, length: 0 };
    ...
    const evenPct = parseFloat(((evenCount / sample.length) * 100).toFixed(1));
    const last20 = ticks.slice(-20).map(t => t.digit ?? getLastDigit(t.quote));
    ...
    res.json({
        ...
        even_odd: { even_pct: evenPct },
        max_streak: maxStreak,
        last_20_digits: last20,
    });
});
...
app.get("/api/balance", async (req, res) => {
    ...
    const timeout = setTimeout(() => {
        ws.close();
        res.status(504).json({ balance: "error", message: "Deriv timeout" });
    }, 10000);
    ...
    ws.onerror = () => {
        clearTimeout(timeout);
        res.status(500).json({ balance: "error", message: "WebSocket error" });
    };
});
